...
public class CommentLikeService {
    ...
    @Transactional
    public CreateCommentLikeResponse create(CustomUserDetails userDetails, Long commentId) {
        ...
        return new CreateCommentLikeResponse(commentLike.getId(), commentLike.getComment().getId(), commentLike.getUser().getId());

    }

    @Transactional
    public List<ReadCommentLikeResponse> read(Long commentId) {
        ...
        List<ReadCommentLikeResponse> dtos = new ArrayList<>();

        for (CommentLike commentLike : commentLikes) {
            if (commentLike.getComment().getId().equals(comment.getId())) {
                dtos.add(new ReadCommentLikeResponse(commentLike.getId(), commentLike.getComment().getId(), commentLike.getUser().getId()));
            }
        }
    ...
    public void delete(Long commentId, CustomUserDetails userDetails) {
        ...
        CommentLike commentLike = commentLikeRepository.findByCommentIdAndUserId(comment.getId(), user.getId()).orElseThrow(
                () -> new CustomException(ErrorCode.COMMENTLIKE_NOT_FOUND)
        );
