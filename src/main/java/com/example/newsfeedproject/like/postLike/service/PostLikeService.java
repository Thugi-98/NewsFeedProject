...
public class PostLikeService {
    ...
    @Transactional
    public PostLikeCreateResponse create(CustomUserDetails userDetails, Long postId) {
        ...
        return new PostLikeCreateResponse(postLike.getId(), postLike.getPost().getId(), postLike.getUser().getId());

    }

    @Transactional
    public List<PostLikeGetAllByPostResponse> read(Long postId) {
        ...
        List<PostLikeGetAllByPostResponse> dtos = new ArrayList<>();

        for (PostLike postLike : postLikes) {
            if (postLike.getPost().getId().equals(post.getId())) {
                dtos.add(new PostLikeGetAllByPostResponse(postLike.getId(), postLike.getPost().getId(), postLike.getUser().getId()));
            }
        }
    ...
    public void delete(Long postId, CustomUserDetails userDetails) {
        ...
        PostLike postLike = postLikeRepository.findByPostIdAndUserId(post.getId(), user.getId())
                .orElseThrow(() -> new CustomException(ErrorCode.POSTLIKE_NOT_FOUND));
