...
public class FollowService {
    ...
    public @Nullable FollowCreateResponse create(CustomUserDetails userDetails, Long targetId) {
        ...
        // 3-1. 내가 나를 팔로우하는 경우 - user.Id와 target.Id가 동일한지 검증
        ...
        return new FollowCreateResponse(follow.getId(), follow.getUser().getId(), follow.getTarget().getId(), follow.getTarget().getName(), follow.getTarget().getEmail());
    }
    ...
    public void delete(CustomUserDetails userDetails, Long targetId) {
        ...
        Follow follow = followRepository.findByUserIdAndTargetId(user.getId(), target.getId())
