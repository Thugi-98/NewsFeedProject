...
public class FollowController {
    ...
    // 다른 사람 팔로우 하기
    @PostMapping
    public ResponseEntity<ApiResponse<FollowCreateResponse>> followApi(
            @RequestParam Long targetId,
            @AuthenticationPrincipal CustomUserDetails user
    ) {
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(ApiResponse.success(followService.create(user, targetId)));
    }

    // userId를 통해 팔로우 목록 확인하기
    @GetMapping
    public ResponseEntity<ApiResponse<List<FollowGetAllByUserResponse>>> followingApi(
            @RequestParam(required = false) Long targetId,
            @AuthenticationPrincipal CustomUserDetails user
    ) {
        return ResponseEntity
                .status(HttpStatus.OK)
                .body(ApiResponse.success(followService.read(user, targetId)));
    }
